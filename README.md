# 台股 Top 100 - 布林通道跟ATR分析 

利用 TEJ 的資料來跑台股市值前 100 大的布林通道跟ATR。

## 目標
計算**布林通道 (相對波動率)** 跟 **ATR (絕對波動率)**，讓大家可以有個動態的參考指標，用來輔助判斷停損或停利的位置。
...
## ATR (Average True Range) 簡介
- 三者取**最大值**：
- **1.當天最高點和最低點間的距離**
- **2.前一天收盤價和當天最高價間的距離**
- **3.前一天收盤價和當天最低價間的距離**
  
- **Momentum**：計算過去 14 天的真實震幅，反映市場熱度。（也可以動態調整）
- **吊燈式追蹤停損 (Chandelier Exit)**：提供 `atr_stop_loss` (High - 3*ATR)，經典的移動停損策略，能讓獲利持續滾動並在大回檔時及時出場。
...
- `window=20`：使用標準的 20 日移動平均。(天數跟標準差都能動態調整）
    - 短線建議：10日均線 ± 1.9標準差
    - 長線建議：50日均線 ±2.1標準差 
...
| **Risk Management** | `atr` (Volatility) |  ATR 數值越高代表市場風險越大，應自動縮小倉位 (避免單筆虧損過大)。 |
| **Chandelier Exit** | `atr_stop_loss` | 當股價從波段最高點回檔超過 3 * ATR 時，立即出場 |
